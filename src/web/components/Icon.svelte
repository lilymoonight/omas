<script lang="ts">
  // Inline-SVG icons (Lucide-style). One component, name prop, no runtime deps.
  // Strokes use currentColor so they inherit text color.
  interface Props {
    name:
      | 'plus' | 'pencil' | 'trash' | 'arrow-left' | 'users' | 'user' | 'user-plus' | 'key' | 'log-out'
      | 'terminal' | 'circle' | 'alert' | 'check' | 'x' | 'refresh'
      | 'monitor' | 'eye-off' | 'wifi' | 'wifi-off'
      | 'cpu' | 'hard-drive' | 'memory' | 'activity' | 'clock'
      | 'git-branch' | 'file' | 'folder' | 'arrow-up' | 'arrow-down'
      | 'chevron-right' | 'chevron-down'
      | 'zap' | 'shield' | 'sparkles' | 'upload'
      | 'sun' | 'moon' | 'search' | 'bell' | 'bell-off' | 'layers'
      | 'globe' | 'copy' | 'download' | 'share' | 'link' | 'command' | 'folder-plus';
    size?: number;
    strokeWidth?: number;
  }
  const { name, size = 16, strokeWidth = 2 }: Props = $props();
