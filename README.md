...
- **免密公开静态站点** — 把任意目录挂到 `/p/<slug>/` 对外免密访问，方便把构建产物、报告等以网页分享给别人或自己回看。**发布管理页**在线增删站点、改动持久化且**无需重启即时生效**；目录 URL 展示带面包屑与按日期分组的**浏览页**（最新日期标 NEW）；未命中文件返回 404；内置路径越界防护（拒绝 `..` / 绝对路径 / NUL）。
