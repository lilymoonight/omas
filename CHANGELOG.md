...
- **公开站点目录页**：无 `index.html` 时自动生成带样式的浏览页——面包屑导航、按修改日期分组（最新在前）、最新日期区块显示 **NEW** 徽章、浅/深色主题。

### 变更

- **公开站点不再探测 `index.html`**：访问目录 URL 一律展示目录页；未命中文件直接 **404**（移除 `--publish-spa` / `spa` 配置与 SPA 回退逻辑）。
...
- **侧栏可折叠，默认收起**：顶栏文件夹 / Git 按钮展开或收起对应侧栏（宽屏 inline、窄屏抽屉）；收起时不挂载面板、不发起文件/Git 刷新 API。

### 修复

- **侧栏与 Git 刷新不及时**：终端 PTY 输出后防抖刷新；Git 面板合并 in-flight 请求避免丢更新；zsh/bash 包装 `cd`/`pushd`/`popd` 即时上报 OSC 7。
- **终端 prompt 被挤出可视区**：修复 `onScroll` 在输出/reflow 时误判「用户上滚」；键盘输入与终端 resize（含侧栏展开）后强制滚到 live screen。

### 变更

- **新建会话默认全宽终端**：PTY 初始列数估算不再预留侧栏宽度（与默认收起一致）。
...
- **OS 级多用户支持（Model B）**：omas 现在可托管多个登录账号，每个账号可映射到一个真实 UNIX 用户，会话以该用户身份运行，由内核的文件权限做隔离。
  - **应用层鉴权，与系统密码无关**：所有账号用 omas 自己的用户名 + argon2id 密码登录（存于配置目录下的 `users.json`，文件权限 0600），不走 PAM、不读系统密码。原单密码部署会自动迁移为一个名为 `admin` 的管理员账号。登录页在存在多个账号时显示用户名输入框；只有一个账号时可省略用户名。
  - **会话归属与全链路鉴权**：每个会话记录 `owner`，普通用户只能看到/操作自己的会话，管理员可见全部。鉴权在 `/api/sessions/:id/*` 用统一的 `preHandler` 与 WebSocket 升级处集中校验。
  - **以目标 UNIX 用户身份降权运行**：服务以 root 运行时，会话 PTY 通过 `runuser`/`sudo -u` 切换到映射用户；Git 子进程（`status`/`diff`/`show` 等）同样以目标用户执行；文件读写经一个会 `initgroups`+`setgid`+`setuid` 降权的一次性 worker 进程完成——root 仅作为执行降权的特权父进程，实际操作均以目标用户身份落地，由内核强制权限。
  - **代建 / 映射 UNIX 用户**：Linux + root 下可代建 UNIX 用户（`useradd --create-home` 并锁定其系统密码，登录仍由 omas 负责）；macOS 仅支持映射到已有用户。删除策略保守：默认只删 omas 账号，UNIX 用户保留，仅对 omas 代建的用户在显式确认（`--purge-os-user`）时才一并删除。
  - **用户管理 CLI**：`omas user add/ls/passwd/rm`，支持 `--os-user <名>` 映射、`--create-os-user` 代建、`--role admin|user`；`connect`/`exec`/`upload`/`download` 客户端命令新增 `--user` 指定登录用户名。
  - **网页用户管理（管理员）**：新增「用户」页可列出/新建/改密/删除账号、映射或解除 UNIX 用户、切换管理员角色（并防止降级或删除最后一个管理员，改密会注销该用户已有会话），改动即时生效无需重启。会话列表头部显示当前登录用户与角色徽章，管理员可在「我的 / 全部」间切换并在会话卡片上看到所属用户。
